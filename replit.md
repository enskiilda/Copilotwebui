...
- Changed sidebar layout from fixed/overlay to true push layout
  - Content shifts right when sidebar opens/closes (no overlay)
  - Removed backdrop overlay for mobile
  - Content uses `min-w-0 overflow-hidden` - elements don't wrap, can be partially hidden
...
### Layout
- **Sidebar**: True push layout - content shifts right when opened, no overlay, elements don't wrap (can be partially hidden)
- **Language**: Default Polish (pl-PL)
...
- Sidebar should push content, not overlay (elements can be partially hidden but don't wrap)
